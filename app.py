...
# Initialize Session States for Persistence across interactions
if 'goals_list' not in st.session_state:
    st.session_state['goals_list'] = [
        {"name": "Emergency Fund", "target": 5000.0, "monthly_target": 300.0},
        {"name": "New Car Down Payment", "target": 8000.0, "monthly_target": 500.0}
    ]
...
# 1. Clean Single-Message Financial Wisdom Banner with Catchy Heading
import random
...
# Sidebar Configuration for Goals & Loans
with st.sidebar:
    ...
    new_goal_target = st.number_input("Target Amount ($)", value=2000.0, step=100.0)
    new_goal_monthly = st.number_input("Monthly Savings Target ($)", value=250.0, step=50.0)
    ...
    st.markdown("---")
    st.subheader("🏦 Loan & EMI Tracker")
    loan_for_input = st.text_input("Loan For (Purpose)", value=st.session_state['loan_saved']['for'])
    loan_principal_input = st.number_input("Total Loan Principal ($)", value=st.session_state['loan_saved']['principal'], step=1000.0)
    loan_emi_input = st.number_input("Monthly EMI ($)", value=st.session_state['loan_saved']['emi'], step=50.0)
    ...
    if st.button("💾 Save Loan Details"):
        st.session_state['loan_saved'] = {
            ...
            "start_date": "2025-10-01" # Baseline start for 12m tracker sync
        }
...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        df = pd.DataFrame({
            ...
            "Amount": [5000.00, 1500.00, 142.50, 17.99, 10.99, 450.00, 120.00, 45.20, 50.00, 85.00],
            "Type": ["Credit", "Debit", "Debit", "Debit", "Debit", "Debit", "Debit", "Debit", "Debit", "Debit"]
        })

    st.dataframe(df, use_container_width=True)

    # Core Cash Flow Math (Renamed Free-to-Spend to Savings This Month)
    ...
    savings_this_month = total_income - total_expenses # User requirement 1
    ...
    # User requirement 2: Reflect goals below expenses and check if monthly goal reached
    if len(st.session_state['goals_list']) == 0:
        st.info("No financial goals added yet. Add some in the sidebar!")
    else:
        goal_cols = st.columns(len(st.session_state['goals_list']))
        for idx, goal in enumerate(st.session_state['goals_list']):
            with goal_cols[idx]:
                target_met = savings_this_month >= goal['monthly_target']
                status_color = "🟢 Target Reached!" if target_met else "🔴 Target Not Met"
                ...
                progress_val = min(float(savings_this_month / goal['monthly_target']), 1.0) if goal['monthly_target'] > 0 else 0.0
...
with tab2:
    ...
    # Mock 12-Month Dataset synchronized with loan parameters
    np.random.seed(42)
    ...
    # User requirement 4: All EMIs aggregated monthly from loan tab
    ...
    # User requirement 3: Calculate months since entry / started and display remaining months in big number format
...
with tab3:
    ...
    if api_key:
        ...
        if user_query:
            ...
            Question: {user_query}
            Provide a concise, direct answer with exact numbers.
            """
